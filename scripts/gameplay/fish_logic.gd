...
static func create_fish(spawn_position: Vector2, fish_type_id: String, fish_config: Dictionary, wander_target: Vector2) -> Dictionary:
	...
	return {
		"pos": spawn_position,
		"velocity": Vector2(randf_range(-GameplayTuning.FISH_INITIAL_VELOCITY_X, GameplayTuning.FISH_INITIAL_VELOCITY_X), randf_range(-GameplayTuning.FISH_INITIAL_VELOCITY_Y, GameplayTuning.FISH_INITIAL_VELOCITY_Y)),
		"wander_target": wander_target,
		"facing": 1.0 if randf() >= 0.5 else -1.0,
		"type": fish_type_id,
		"growth": 0.0,
		"hunger": GameplayTuning.FISH_INITIAL_HUNGER,
		"coin_timer": fish_config["coin_interval"],
		"guard_cooldown": randf_range(GameplayTuning.GUARD_INITIAL_COOLDOWN_MIN, GameplayTuning.GUARD_INITIAL_COOLDOWN_MAX),
		"hp": max_hp,
		"max_hp": max_hp,
		"alive": true,
	}
...
static func try_update_feeding(fish: Dictionary, fish_config: Dictionary, food_list: Array[Dictionary], target_food_index: int, fish_index: int, claim_winner_index: int, fish_radius: float, food_radius: float, growth_bonus_multiplier: float = 1.0) -> bool:
	...
	fish["velocity"] = direction * GameplayTuning.FISH_FEED_SPEED
	if fish["pos"].distance_to(target_food["pos"]) < fish_radius + food_radius and fish_index == claim_winner_index:
		fish["hunger"] = min(GameplayTuning.FISH_MAX_HUNGER, fish["hunger"] + GameplayTuning.FISH_FEED_BASE_HUNGER_GAIN + target_food["nutrition"] * GameplayTuning.FISH_FEED_NUTRITION_HUNGER_GAIN)
		fish["growth"] = min(1.0, fish["growth"] + GameplayTuning.FISH_FEED_GROWTH_GAIN * target_food["nutrition"] * fish_config["growth_multiplier"] * growth_bonus_multiplier)
...
static func try_update_evasion(fish: Dictionary, fish_config: Dictionary, enemy_position: Vector2, evasion_radius: float, full_strength_radius: float, evasion_speed: float, minimum_blend: float, maximum_blend: float) -> bool:
	...
	if current_velocity.dot(escape_direction) < 0.0:
		blend = max(blend, GameplayTuning.FISH_EVASION_REVERSE_MINIMUM_BLEND)
...
static func update_guard_movement(fish: Dictionary, enemy_position: Vector2, attack_range: float) -> void:
	...
	if distance <= 1.0:
		fish["velocity"] = fish["velocity"].lerp(Vector2.ZERO, GameplayTuning.GUARD_STOP_BLEND)
		return
	var direction := to_enemy / distance
	var preferred_distance := attack_range * GameplayTuning.GUARD_PREFERRED_RANGE_RATIO
	if distance > preferred_distance:
		fish["velocity"] = fish["velocity"].lerp(direction * GameplayTuning.GUARD_APPROACH_SPEED, GameplayTuning.GUARD_APPROACH_BLEND)
	else:
		var orbit_direction := Vector2(-direction.y, direction.x)
		fish["velocity"] = fish["velocity"].lerp(orbit_direction * GameplayTuning.GUARD_ORBIT_SPEED, GameplayTuning.GUARD_ORBIT_BLEND)
